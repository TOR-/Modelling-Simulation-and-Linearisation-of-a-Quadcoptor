...
In our model, gravitational acceleration was taken to be a constant irrespective of altitude. This is not realistic, but a justified in that within the region of operation, gravitational acceleration is very close to constant. We are assuming very small changes in altitude (see \cref{s:simp g force}).
...
\begin{align}
\mat{Q}=\begin{bmatrix}
\cos(\psi)\cos(\theta) & -\sin(\theta)\cos(\phi)+\cos(\psi)\sin(\theta)\sin(\phi) & \sin(\psi)\sin(\phi)+\cos(\psi)\sin(\theta)\cos(\phi)\\
